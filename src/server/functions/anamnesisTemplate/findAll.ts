...
import { makeControllerFindAllAnamnesisTemplate } from '@/factories/controllers/anamnesisTemplate/makeControllerFindAllAnamnesisTemplate';
...
export async function handler(event: APIGatewayProxyEventV2) {
  ...
  const response = await controllerAdapter(
    makeControllerFindAllAnamnesisTemplate(),
